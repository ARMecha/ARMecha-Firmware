...
pub(crate) fn gen_build() {
    ...
    let contents = fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/keyboards/handwired/BMMP/Config.toml")).unwrap();
    ...
    for fname in ControllerConfig::field_names().iter() {
        let meta = &decoded.mcu.gen_meta_tuple(fname);
        let const_out = format!(
            "pub const {}_{}: {} = {};\n",
            meta.0.to_uppercase().split("CONFIG").next().unwrap(),
            meta.1.to_uppercase(),
    ...
    for fname in DynamicConfig::field_names().iter() {
        let meta = &decoded.dynamic_keymap.gen_meta_tuple(fname);
        let const_out = format!(
            "pub const {}_{}: {} = {};\n",
            meta.0.to_uppercase().split("CONFIG").next().unwrap(),
            meta.1.to_uppercase(),
